...
| 名前                 | ユーザー名                            | パスワード | 役職                     | 部               |
| -------------------- | ------------------------------------- | ---------- | ------------------------ | ---------------- |
| エドモンド・リーブ   | `ereeve@yourtenant.onmicrosoft.com`   | Pa55-w.rd! | 人事担当者               | 人事             |
| ミランダ・スナイダー | `msnider@yourtenant.onmicrosoft.com`  | Pa55-w.rd! | ヘルプデスクマネージャー | オペレーションズ |
| コーディ・ゴディネス | `cgodinez@yourtenant.onmicrosoft.com` | Pa55-w.rd! | 営業担当                 | セールス         |
...
7. [パスワードの保存] プロンプトで、[**保存してオンにする**] を選択します。

8. [サインインしたままにする] プロンプトで、[**いいえ**] を選択します。Entra 管理センターが開きます。

9. Microsoft Entra 管理センターのナビゲーション ウィンドウで、 **[ユーザー]** を選択します。
...
10. **ユーザー |[すべてのユーザー]** ページで、[**新しいユーザー**] を選択し、[**新しいユーザーの作成**] を選択します。

11. [**新しいユーザーの作成**] ページで、次のように入力します。

    - ユーザープリンシパル名: **ereeve**
    - 表示名: **エドモンド・リーブ**

12. **自動生成されたパスワードの**チェックを外します

13. [**パスワード]** の横に「**Pa55-w.rd!」**と入力します。

    > 注: このパスワードが弱い、または一般的に使用されるというエラーメッセージが表示された場合は、このラボプロファイルの **[リソース]** タブにあるテナントパスワードを入力します。または、選択した複雑なパスワードを入力することもできます。

14. ページの下部にある **[次へ: プロパティ]** を選択します。

15. [**名**] の横に「**Edmund**」と入力します。

16. [**姓**] の横に「**Reeve**」と入力します。

17. [**ユーザータイプ**] の横で、[**メンバー]** が選択されていることに注意してください。

    > 注: **メンバー** ユーザー タイプは、デフォルトのユーザー タイプです。このユーザータイプは、組織内のほとんどのユーザーに使用されます。

18. [**役職]** の横に「**HR Rep**」と入力します。

19. **[部門]** の横に「**HR」**と入力します。

20. [**使用場所]** の横にある [**米国**] を選択します。

21. ページの下部にある **[次へ: 割り当て**] を選択します。

22. [**割り当て]** ページで、割り当てが選択されていないことに注意してください。

    > デフォルトでは、グループはユーザーに割り当てられません。これは、ユーザーが割り当てるまで、そのユーザーはどのグループのメンバーでもないためです。

23. ページの下部にある **[次へ: 確認 + 作成**] を選択します。

    > このページの情報が正しいことを確認してください。

24. **[作成]** を選択します。

25. **ユーザー |[すべてのユーザー]** ページで、[**新しいユーザー**] を選択し、[**新しいユーザーの作成**] を選択します。

26. [**新しいユーザーの作成**] ページで、次のように入力します。

    - ユーザー プリンシパル名: **msnider**
    - 表示名: **ミランダ・スナイダー**

27. **自動生成されたパスワードの**チェックを外します

28. [**パスワード]** の横に「**Pa55-w.rd!」**と入力します。

    > 注: このパスワードが弱い、または一般的に使用されるというエラーメッセージが表示された場合は、このラボプロファイルの **[リソース]** タブにあるテナントパスワードを入力します。または、選択した複雑なパスワードを入力することもできます。

29. ページの下部にある **[次へ: プロパティ]** を選択します。

30. [**名**] の横に「**Miranda**」と入力します。

31. [**姓**] の横に「**Snider**」と入力します。

32. [**ユーザータイプ**] の横で、[**メンバー]** が選択されていることに注意してください。

33. [**役職]** の横に「**ヘルプデスク マネージャー**」と入力します。

34. **[部門]** の横に「**オペレーション」**と入力します。

35. [**使用場所]** の横にある [**米国**] を選択します。

36. ページの下部にある **[次へ: 割り当て**] を選択します。

37. [**割り当て]** ページで、割り当てが選択されていないことに注意してください。

38. ページの下部にある **[次へ: 確認 + 作成**] を選択します。

39. **[作成]** を選択します。

40. **Microsoft Edge** ウィンドウを最小化します。
...
4. **PowerShell-7.5.2-win-x64.msi**をダブルクリックして、セットアップウィザードを起動します。
   ...
   - **[次へ**] を選択します
   - **宛先フォルダー**をそのままにして、[**次へ**] を選択します
   - **オプションのアクション**をそのままにして、[**次へ**] を選択します
   - [*Microsoft Update を使用してコンピューターをセキュリティで保護し、最新の状態に保つ]* のチェック ボックスを空白のままにして、[**次へ**] を選択します
   - インストール **を選択します**

5. [**インストールが正常に完了しました]** ウィンドウで、[**PowerShell の起動]** をオンにして、[**完了]** を選択します。
...
8. **Microsoft Edge** に新しいタブが表示され、サインインを求められます。[**アカウントにサインイン]** ダイアログ ボックスで、テナント パスワードを使用して **`admin@yourtenant.onmicrosoft.com`** としてサインインし、 **[サインイン]** を選択します。

9. 表示される **[要求されたアクセス許可]** プロンプトで、[**組織に代わって同意**] をオンにし、[**同意する]** を選択します。

10. [**認証の完了]** タブを閉じて、**Microsoft Edge** を最小化します

11. 戻る **PowerShell 7** ウィンドウで、次のコードを入力して新しいプロファイル オブジェクトを作成し、**Enter キー**を押します。**Pa55w.rd** を任意の複雑なパスワードに置き換えます。
...
| -------------------- | -------------------------------------------------- | ------------------ |
| アラン・デヨン       | テナントを管理する                                 | グローバル管理者   |
| エドモンド・リーブ   | ユーザー、グループ、パスワードのリセットを管理する | ユーザー管理者     |
| ミランダ・スナイダー | パスワードのリセットを管理する                     | ヘルプデスク管理者 |
...
2. Microsoft Entra 管理センターの [ナビゲーション] ウィンドウで、[**ロールと管理者**] を選択します。
...
8. ナビゲーションの階層リンクで、[**ロールと管理者] |[すべてのロール] を選択します**。

9. 検索ボックスを使用して、**ユーザー管理者**を検索します。

10. [**ユーザー管理者**] を選択します。

11. [**ユーザー管理者**] ウィンドウで、[**+ 割り当ての追加**] を選択します。

12. [**メンバーの選択**] で [**メンバーが選択されていません]** を選択し、[**Edmund Reeve]** を検索して選択します。

13. [**追加]** を選択します。

14. ナビゲーションの階層リンクで、[**ロールと管理者] |[すべてのロール] を選択します**。

15. 検索ボックスを使用して、**ヘルプデスク管理者**を検索します。

16. [**ヘルプデスク管理者]** を選択します。

17. **[ヘルプデスク管理者]** ウィンドウで、[**+ 割り当ての追加**] を選択します

18. **[メンバーの選択**] で [**メンバーが選択されていません**] を選択し、**Miranda Snider** を検索して選択します。

19. [**追加]** を選択します。

20. ナビゲーション ウィンドウで、**ホーム を選択します。**
...
| 名前                 | メンバー:        | 割り当てるライセンス                                         |
| -------------------- | ---------------- | ------------------------------------------------------------ |
| エドモンド・リーブ   | Contoso_Managers | Office 365 E5、Enterprise Mobility + Security E5 (グループ メンバーシップ経由) |
| ミランダ・スナイダー | Contoso_Managers | Office 365 E5、Enterprise Mobility + Security E5 (グループ メンバーシップ経由) |
| コーディ・ゴディネス | Contoso_Sales    | Office 365 E5、Enterprise Mobility + Security E5 (グループ メンバーシップの直接割り当てによる) |
...
1. **SEA-SVR1** の Microsoft Entra 管理センターのナビゲーション ウィンドウで、 **[グループ]** を選択します。
2. [**新しいグループ**] を選択します。
...
11. 左側のナビゲーション ウィンドウで、 [**ユーザー**] > **[アクティブ ユーザー**] を選択します。

12. ユーザリストで、**Cody Godinez** を選択します (チェックボックスではなく名前を選択します)。

13. [**ライセンスとアプリ]** タブを選択します。

14. [**エンタープライズ モビリティ + セキュリティ E5**] と **[Office 365 E5 (Teams なし)]** の横にあるチェック ボックスをオンにします。

15. **[変更の保存]** を選択します。

16. 変更が保存されたら、右上隅の **[X**] を選択して **[Cody Godinez**] ウィンドウを閉じます。

17. Microsoft 365 管理センターの [ナビゲーション] ウィンドウで、[**課金**>**ライセンス**] を選択します。

18. [**サブスクリプション]** の一覧で、[**エンタープライズ モビリティ + セキュリティ E5**] を選択します。
...
22. [**ライセンスを割り当てたContoso_Managers**] ウィンドウで、右上隅の **[X**] を選択して閉じます。

23. **[Enterprise Mobility + Security E5**] ページの左上隅にある [**ライセンスに戻る]** リンクを選択します。

24. [**サブスクリプション]** の一覧で、[**Office 365 E5 (Teams なし)]** を選択します。
...
28. [**ライセンスを割り当てたContoso_Managers**] ウィンドウで、右上隅の **[X**] を選択して閉じます。

29. Microsoft 365 管理センターの [ナビゲーション] ウィンドウで、[**課金**>**ライセンス**] を選択します。

30. [**サブスクリプション]** の一覧で、[**Office 365 E5 (Teams なし)]** を選択します。

> Office 365 E5 ライセンスが割り当てられているユーザーをメモします。Edmund と Miranda はどちらも、Contoso_Managers グループのメンバーシップからライセンスの割り当てを受け取ります。[**グループ]** タブを選択して、ライセンスが正しく割り当てられているかどうかを確認できます。ライセンスの再処理には3〜5分かかる場合があります。
